...
class ChatApp:
    def __init__(self, root):
        ...
        self.text_area.grid(column=0, row=1, padx=10, pady=10, sticky="nsew", columnspan=4)

        # 创建一个输入框
        self.entry = tk.Entry(self.root)
        self.entry.grid(column=0, row=4, padx=10, pady=10, sticky="ew", columnspan=4)

        # 创建一个发送按钮
        self.send_button = tk.Button(self.root, text="发送", command=self.send_message)
        self.send_button.grid(column=0, row=5, padx=10, pady=10, sticky="ew")

        # 创建一个debug按钮
        self.debug_button = tk.Button(self.root, text="Debug", command=self.show_debug_info)
        self.debug_button.grid(column=1, row=5, padx=10, pady=10, sticky="ew")
        ...
        self.stream_switch.grid(column=0, row=3, padx=10, pady=5, sticky="w", columnspan=1)

        # 创建上下文模式选择下拉菜单
        self.context_mode_var = tk.StringVar(value="上下文连续对话")
        self.context_mode_menu = tk.OptionMenu(self.root, self.context_mode_var, "独立对话", "上下文连续对话")
        self.context_mode_menu.grid(column=1, row=3, padx=10, pady=5, sticky="w", columnspan=3)
